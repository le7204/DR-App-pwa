...
export default class GrowItemsList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      growItems: false
  ...
  componentDidMount() {
    ...
    try {
      client.request(getGrowItemList).then((data) => {
        console.log(data.growItems);
        this.setState({ growItems: [...data.growItems], loaded: true });
      });
    } catch (err) {
      console.error('call to cms for grow items failed:', err);
    }
  }
  render() {
    return <div className="grow-item-list" style={{ height: '100vh' }}>
      {!this.state.growItems && <Loading />}
      {this.state.growItems && this.state.growItems.map((element, i) => {
        // console.log(element)
        return <button key={"item" + i} onClick={() => this.props.growItemClick(element)}>{element.name}</button>;
      })}
    </div>;
  }
}
