...
const HousingItemsList = () => {
  ...
  useEffect(() => {
    ...
    try {
      client.request(getHousingItemList).then((data) => {
        console.log(data.housingItems);
        setState({ housingItems: [...data.housingItems], loaded: true });
      });
    } catch (err) {
      console.error('call to cms for housing items failed:', err);
    }
  }, []);

  const navigate = (housingItem) => {
    console.log("navigating:", housingItem);
