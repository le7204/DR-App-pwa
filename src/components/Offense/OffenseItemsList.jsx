...
const OffenseItemsList = () => {
  ...
  useEffect(() => {
    ...
    try {
      client.request(getOffenseItemList).then((data) => {
        console.log(data.offenseItems);
        setState({ offenseItems: [...data.offenseItems], loaded: true });
      });
    } catch (err) {
      console.error('call to cms for offense items failed:', err);
    }
  }, []);

  const navigate = (offenseItem) => {
    console.log("navigating:", offenseItem);
