...
const CrewItemsList = () => {
  ...
  useEffect(() => {
    ...
    try {
      client.request(getCrewItemList).then((data) => {
        console.log(data.cartelItems);
        setState({ crewItems: [...data.cartelItems], loaded: true });
      });
    } catch (err) {
  ...
  const navigate = (crewItem) => {
    console.log("navigating:", crewItem);
    nav(`/crewitem/${crewItem}`);
  };
  return (
    <div className="crew-item-list" style={{ height: '100vh' }}>
      {!state.crewItems && <Loading />}
      {state.crewItems && state.crewItems.map((element, i) => {
        // console.log(element)
