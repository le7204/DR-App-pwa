...
const PrestigeItemsList = () => {
  ...
  useEffect(() => {
    ...
    try {
      client.request(getPrestigeItemList).then((data) => {
        console.log(data.prestigeItems);
        setState({ prestigeItems: [...data.prestigeItems], loaded: true });
      });
    } catch (err) {
      console.error('call to cms for prestige items failed:', err);
    }
  }, []);

  const navigate = (prestigeItem) => {
    console.log("navigating:", prestigeItem);
...
export default PrestigeItemsList;
